...
- **Grandfathered terms (#5):** each position stores `positionPenaltyCap`, snapshotted from the global `maxPenaltyBps` at creation. Effective cap = `min(positionPenaltyCap, current maxPenaltyBps)` — global *reductions* benefit everyone immediately; global *increases* never worsen an existing commitment. Soulbound + penalty-as-only-door makes this essential: exit economics must be predictable at lock time.
- **`increase_amount` rule (loophole the grandfathering would otherwise open):** if added principal simply inherited an old low cap, a user could park a tiny position under cheap terms and later pour size into it for discounted exits. Fix: on `increase_amount`, the cap re-weights — `newCap = (oldPrincipal × oldCap + addedPrincipal × currentGlobal) / newPrincipal`. Old principal keeps its terms exactly; new principal enters at current terms; nothing is gameable and nothing is silently worsened. `autoCompound`'s small weekly adds drift the cap only marginally and only toward current terms, which is fair.
